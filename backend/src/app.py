...
async def get_tasks(
    ...
    priority: Optional[TaskPriority] = None,
    assignee: Optional[str] = None,
    db: Session = Depends(get_db) # ← Toujours en dernier dans les param
):
    ...
    return db.query(TaskModel).all()      #query.all()
...
async def create_task(task_data: TaskCreate,db: Session = Depends(get_db)) -> Task:
    ...
    task=TaskModel(id=str(uuid.uuid4()),title=task_data.title,
...
async def update_task(task_id: str, updates: TaskUpdate, db: Session = Depends(get_db)) -> Task:
    ...
    setattr(task, "title", updates.title)
    setattr(task, "description", updates.description)
    setattr(task, "status", updates.status)
    setattr(task, "priority", updates.priority)
    setattr(task, "due_date", updates.due_date)
    setattr(task, "assignee", updates.assignee)
    setattr(task, "update_at", now)
